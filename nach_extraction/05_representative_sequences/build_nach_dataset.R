...
td_dir  <- "04_orf_prediction_and_clustering/outputs/transdecoder/Transdecoder/p1_noflag"
ref_pep <- "Reference/sodium_channel/refs_84.fasta"
out_dir <- "05_representative_sequences/outputs/representatives"
...
read_pep_file <- function(f) {
  ...
  # Normalize to zero-padded form: CF_3 -> CF003
...
# Priority: keep complete ORFs when available
# If a sample has no complete ORF, use the strongest remaining candidate by score then length
orf_rank <- c("complete" = 1, "5prime_partial" = 2, "3prime_partial" = 3, "internal" = 4)

reps <- all_meta %>%
  mutate(orf_rank = orf_rank[orf_type]) %>%
  group_by(sample_id) %>%
  mutate(has_complete = any(orf_type == "complete")) %>%
  filter(if_else(has_complete, orf_type == "complete", TRUE)) %>%
  arrange(desc(score), desc(len), .by_group = TRUE) %>%
  dplyr::slice(1) %>%
  ungroup() %>%
  select(-has_complete) %>%
...
# Trim seq_id in seqs to match parsed seq_id (Biostrings uses full header as name)
